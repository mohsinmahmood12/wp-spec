...
function html_import_get_options() {
	$defaults = array(
		'root_directory' => ABSPATH.__( 'static-html-spdx-specs-files-to-import', 'import-html-pages' ),
		'old_url' => '',
...
function html_import_options_page() { ?>
	...
	<div class="ui-tabs">
		...
		<h2><?php _e( 'SPDX Specs Settings', 'import-html-pages' ); ?></h2>
		<?php
		if ( $options['firstrun'] === true ) {
		echo '<p>'.sprintf( __( 'Welcome to SPDX SPecs! Please look through all the tabs on this page before running your import.', 'import-html-pages' ), 'options-general.php?page=html-import.php' ).'</p>';
		}
		?>
...
function html_import_validate_options( $input ) {
	...
	if ( validate_import_file( $input['root_directory'] ) > 0 ) {
		...
		$input['root_directory'] = ABSPATH.__( 'static-html-spdx-specs-files-to-import', 'import-html-pages' );
	}

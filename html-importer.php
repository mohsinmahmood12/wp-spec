...
class HTML_Import extends WP_Importer {
	...
	function header() {
		echo '<div class="wrap">';
		screen_icon();
		echo '<h2>'.__( 'SPDX Specs', 'import-html-pages' ).'</h2>';
	}
	...
	function greet() {
		...
		echo '<p>'.sprintf( __( 'It looks like you have not yet visited the <a href="%s">SPDX Specs options page</a>. Please do so now! You need to specify which portions of your HTML files should be imported before you proceed.', 'import-html-pages' ), 'options-general.php?page=html-import.php' ).'</p>';
		}
		else { ?>
	...
	function get_post( $path = '', $placeholder = false ) {
		...
		if ( $placeholder ) {
			...
			else if ( $options['import_content'] == "file" ) {
